...
async def get_notice_info(cp_id: str, app_id: str, org_id: str, org_key: str, org_secret: str):
    # Verify org_key and org_secret
    organisation = developer_details_collection.find_one(
        {"organisation_id": org_id, "org_key": org_key, "org_secret": org_secret}
    )
    ...
    if not collection_point:
        raise HTTPException(status_code=404, detail=f"Collection point with ID {cp_id} not found")
    ...
    notice_info["french"]["collection_point"] = collection_point_info
    notice_info["spanish"]["collection_point"] = collection_point_info
    notice_info["german"]["collection_point"] = collection_point_info
